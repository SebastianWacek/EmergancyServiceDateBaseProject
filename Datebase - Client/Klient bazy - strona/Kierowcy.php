...
<div class="artykul">
 ...
 <?php
	...
	if (!$conn) {
		...
		$stid = oci_parse($conn, 'SELECT id_kierowcy, imie, nazwisko, wiek, lata_stazu, id_karetki FROM kierowcy');
		
		oci_execute($stid);
		
		echo "<table border='1'>";
		echo "<tr><th>" . "ID kierowcy" . "</th><th>" . "Imie" . "</th><th>" . "Nazwisko" . "</th><th>" . "Wiek" . "</th><th>" . "Lata stazu" . "</th><th>" . "ID karetki" . "</th></tr>";
		
		while(($row = oci_fetch_array($stid, OCI_BOTH))!=false){
		echo "<tr><td>" . $row['ID_KIEROWCY'] . "</td><td>" . $row['IMIE'] . "</td><td>" . $row['NAZWISKO'] . "</td><td> " . $row['WIEK'] . "</td><td> " . $row['LATA_STAZU'] ."</td><td>" . $row['ID_KARETKI'] . "</td></tr>";
		}
		echo "</table>";
	}
	oci_close($conn);
...
<center>
<b>Dodaj kierowce</b>
 <form enctype="multipart/form-data" action="Kierowcy.php" method="post">
        ...
        <p>Lata stażu:  <input type="text" id="name" 				name="zm5" maxlength="20" /></p>
...
if (!$conn) {
		...
		$sql = 'INSERT INTO kierowcy (id_kierowcy, imie, nazwisko, wiek, lata_stazu, id_karetki) '. 'VALUES(:v1, :v2, :v3, :v4, :v5, :v6)';
