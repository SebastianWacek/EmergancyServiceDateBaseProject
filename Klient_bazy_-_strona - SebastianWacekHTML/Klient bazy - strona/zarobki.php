...
<div class="artykul">
 ...
 <?php
	...
	if (!$conn) {
		...
		$stid = oci_parse($conn, 'SELECT id_wynagrodzenia, wysokosc_wynagrodzenia, sposob_naliczania, wysokosc_premii, id_dyzuru FROM zarobki');
		
		oci_execute($stid);
		
		echo "<table border='1'>";
		echo "<tr><th>" . "ID wynagrodzenia" . "</th><th>" . "Wysokosc wynagrodzenia" . "</th><th>" . "Sposób naliczania" . "</th><th>" . "Wysokość premii" . "</th><th>" . "ID dyżuru" . "</th></tr>";
		
		while(($row = oci_fetch_array($stid, OCI_BOTH))!=false){
		echo "<tr><td>" . $row['ID_WYNAGRODZENIA'] . "</td><td>" . $row['WYSOKOSC_WYNAGRODZENIA'] . "</td><td>" . $row['SPOSOB_NALICZANIA'] . "</td><td> " . $row['WYSOKOSC_PREMII'] . "</td><td> " . $row['ID_DYZURU'] ."</td></tr>";
		}
		echo "</table>";
	}
	oci_close($conn);
...
<center>
<b>Informacje o zarobkach</b>
 <form enctype="multipart/form-data" action="zarobki.php" method="post">
        <p>Id:  <input type="text" id="name" 						name="zm1" maxlength="20" /></p>
        <p>Wysokość wynagrodzenia:  <input type="text" id="name" 	name="zm2" maxlength="15" /></p>
        <p>Sposób naliczania:  <input type="text" id="name" 				name="zm3" maxlength="20" /></p>
        <p>Wysokość premii:  <input type="text" id="name"	name="zm4" maxlength="20" /></p>
...
$conn = oci_connect("pogotowie_ratunkowe", "7890", "//localhost/XEPDB1", "AL32UTF8");
	if (!$conn) {
		...
		$sql = 'INSERT INTO zarobki (id_wynagrodzenia, wysokosc_wynagrodzenia, sposob_naliczania, wysokosc_premii, id_dyzuru) '. 'VALUES(:v1, :v2, :v3, :v4, :v5)';
